...
# ╔═╡ 95f4677e-6c65-4684-9734-ba11c6073fd3
begin
...
# ╔═╡ ce1fe745-1ab2-4cbb-98c5-cc1a9d31e090
md"""


*Load required packages for the notebook. It might take up to 10 mins. Hang in there!*

"""

# ╔═╡ e02271e3-6135-4999-92bc-2bb263af7c8c
md"""
*Set auto-differentiation backend for Turing. Reverse differentiation is faster for larger models with a large size of parameters. Turing.jl is used for demonstration purposes.*
"""

# ╔═╡ 685dc7f2-07b0-4d8d-96db-995456a12f7b
Turing.setadbackend(:reversediff)

# ╔═╡ ec60d164-72b7-442b-9f68-00e538fafbb2
md"""
# Variational inference
##### Lei Fang (lf28@st-andrews.ac.uk)
"""

# ╔═╡ 90b70f12-a723-4252-95d5-1cb917fc3ec8
md"""
**Abstract** This Julia notebook replicates the result of Fig 4 in the paper titled *The variational approximation for Bayesian inference* in IEEE Signal Processing Magazine, by *D. G. Tzikas, A. C. Likas and N. P. Galatsanos*. Besides the algorithms presented in the paper, the notebook has also implemented the following extra inference algorithms:
* Gibbs sampling inference for relevant vector regression model
* a full Bayesian inference on RVM, which includes inference on the basis bandwidth parameter $\sigma_{\Phi}^2$
* RVM model by Turing.jl
Written for teaching purposes. 
"""

# ╔═╡ 0e2d95de-48b2-4de8-9a08-303aa034941a
md"""
...
# ╔═╡ 612d69a9-dd51-402e-a21b-d8cff74d7662
begin
	...
	xsamples_ = collect(range(-10., 10., length=2*N))
	xsamples = shuffle(xsamples_)[1:N]
	# generate targets or y
	ts = signal_.(xsamples) + rand(Normal(0, sqrt(σ²)), N)
	print()
end

# ╔═╡ d52969db-e5ed-42ce-80fa-1c6069b569f0
begin
	...
	# 	n_basis = size(xtrain)[1] 
	# 	n_obs = size(xtest)[1]
	# 	X = zeros(n_obs, n_basis) 
	# 	for j in 1:n_basis
	# 		X[:,j] = rbf_basis.(xtest, xtrain[j], σ²)
	# 	end
	# 	intercept ? [ones(n_obs) X] : X
	# end
end

# ╔═╡ 577e80df-06c6-4f01-afb6-aca90e5b50ad
begin
	intercept = true
	σϕ²= 0.5
...
# ╔═╡ d58a538d-a60c-485e-9118-347dc59d5457
md"""
RBF basis functions
"""

# ╔═╡ 0cd0c1c7-54bc-47f7-a907-213b82de51dd
md"""
...
# ╔═╡ d1108900-af0f-40c3-822e-a4695ffe96af
begin
	# ML estimation by normal equation
	# wₘₗ = (Φ'*Φ)^(-1)*Φ'*ts
	function mlLR(xtest, xtrain, tt, σ² = 1, intercept=false)
		Φ_ = basis_expansion(xtrain, xtrain, σ², intercept)
		wₘₗ = (Φ_'*Φ_)^(-1)*Φ_'*tt
		# predict(xtest, wₘₗ, xtrain, σ², intercept)
	end
end

# ╔═╡ f9f2c296-1aa8-472d-ad9b-74929288a23d
md"""
## EM algorithm
"""

# ╔═╡ 57f43f2e-c3a3-4f94-a81f-e2cbe6b3b0fb
begin
	function EMBayesianLR(Φ, ts, maxIters = 100, tol = 1e-4)
		N,M = size(Φ)
		innerM = Φ'*Φ
		scatterM = Φ * Φ'
		# initial α and β
		αₜ = βₜ = 1.0 
		invΣₜ = βₜ .* innerM + αₜ .* Matrix(1.0I, M, M)
		Σₜ = invΣₜ^(-1)
		μₜ = βₜ .* (Σₜ * Φ' * ts)
		logLiks = zeros(maxIters)
		iter = 1
		while iter < maxIters
			# M step
			αₜ = M / (sum(μₜ.^2) + tr(Σₜ))
			error = ts - Φ*μₜ
			# note the equation given in the paper is wrong! 
			βₜ = N / (sum(error.^2) + tr(Φ'Φ*Σₜ))
			# E step
			invΣₜ = βₜ .* innerM + αₜ .* Matrix(1.0I, M, M)
			Σₜ = invΣₜ^(-1)
			μₜ = βₜ .* (Σₜ * Φ' * ts)
			logLiks[iter] = logpdf(MvNormal(Diagonal(βₜ^(-1) .* ones(N)) + αₜ^(-1) .* scatterM), ts)
			if (iter >1) && (abs(logLiks[iter] - logLiks[iter-1]) < tol)
				iter = iter + 1
				break
			end
			iter = iter + 1
		end
		return μₜ, Σₜ, αₜ, βₜ, logLiks[1:iter-1]
	end

end

# ╔═╡ b20e64fd-f9f1-4182-a305-80567695ea5a
wₑₘ,_,αₑₘ,βₑₘ,logLiks=EMBayesianLR(Φ, ts);

# ╔═╡ ffc776b4-d16b-4655-a7de-ebcabebbd509
plot(logLiks, label="Log Lik", xlabel="iteration")

# ╔═╡ f143422d-2ff2-4d73-a8a8-05ca58e42d71
md"""
...
# ╔═╡ d0eac675-5f2c-4b0a-8cdd-bc837239ddef
...
# ╔═╡ 06fb4753-10e4-49bd-8ad5-09f57117c568
begin
	a0 = 1e-6
	b0 = 1e-6
	c0 = 1e-6
...
# ╔═╡ 2f668109-26b1-43f5-a6f8-14d0934b7056
md"""

The "relevant" vectors are associated with those non-zero weights.
"""

# ╔═╡ 876e55b0-fc58-4a42-9211-9ee301e92fce
plot(wᵥᵢ, label="μ")

# ╔═╡ bccdfdb8-1a18-4f9d-b753-28290177195e
md"""

## Gibbs sampling

To draw a comparison between Gibbs sampling and Variational inference, I have implemented a Gibbs sampling based inference algorithm of the RVM.
"""

# ╔═╡ 8ebd0462-49a2-4ffd-8e1a-6ed5adf614b4
begin
# Gibbs sampling solution

	function samplew(t, Φ, m0, C0, β)
		C0Inv = inv(C0)
		CnInv = C0Inv + β * (Φ'*Φ)
		Cn = inv(CnInv)
		mn = Cn*(β*Φ'*t + C0Inv * m0)
...
# ╔═╡ e4f6e619-38ed-4601-8bfb-3755e7c9cfe5
begin
	ws, αs, βs=gibbsBayesianLR(Φ, ts, a0, b0, c0, d0);
	# gibbsPrediction = mean(predict(xs, ws, xsamples, σϕ², intercept), dims=2)
	plot(ws[:, 1000:100:end], title="w samples", label="")
end

# ╔═╡ f2ec6b6c-9de1-48c6-8b0c-fdd9c19232ac
md"""

Larger $\alpha$ values signal the feature is shut-off. 
"""

# ╔═╡ b34d8e28-740e-47b7-9e71-bb7a6af4c8b5
plot(αs[1:2:end, 1000:10:end]', label="", title="α samples")

# ╔═╡ cc03ead4-1ac3-477f-9929-39b1064247c0
md"""

## Full Bayesian inference on the RVM

The inference now includes the unknown basis expansion's bandwidth, $\sigma^2_{\Phi}$, which controls the smoothness of the regression function. 
"""

# ╔═╡ e2ef9e1e-7ec9-4e00-97ed-fca1a19e8604
md"""
The posterior samples of the bandwidth from the Gibbs sampling.
"""

# ╔═╡ 2dc217e3-f156-4585-b0a3-e905bc1b0e12
md"""
## RVM by Turing.jl

The inference can take up to 10 mins!
"""

# ╔═╡ 6cd092b9-e54f-4147-b42b-2086d1d365d8
...
@model function linear_regression(x, y)
    ...
    nfeatures = size(x, 2)
	# α = Vector{Real}(undef, nfeatures)
	# for i in 1:nfeatures
	# 	α[i] ~ truncated(Normal(0, 100), 0, Inf)
	# end
	# α ~ filldist(Gamma(1e-1, 1/1e-1), nfeatures)
	# Turing struggles if the a0 b0 set as 0
	α ~ filldist(truncated(Gamma(1, 1), 0, 5000), nfeatures)
    coefficients ~ MvNormal(zeros(nfeatures), Diagonal(α))

    # Calculate all the mu terms.
    # mu = intercept .+ x * coefficients
	mu = x * coefficients
    y ~ MvNormal(mu, sqrt(1/β))
end

# ╔═╡ cd4db460-ecf4-43c0-b30f-80ee6f2b3225
begin
	model_ = linear_regression(Φ, ts)
	chain_ = sample(model_, NUTS(0.65), 3_000);
end

# ╔═╡ fdc3dd63-d164-4f54-9dcb-526c9d34a96b
md"""
...
# ╔═╡ 71adf40e-8c60-443f-8a60-0e154c02670a
md"""

#### Bayesian prediction by Gibbs sampling

"""

# ╔═╡ 1f87d81a-a322-4850-baeb-b756f76a45de
md"""
...
# ╔═╡ af2e93be-e650-435b-9098-5a9d703b56d3
begin
	# prediction method for full bayesian approach
	function predict_basis(xtest, ws, σ²s , xtrain, intercept=false)
		mc = size(σ²s)[1]
		N = length(xtest)
		ts = zeros(N,mc)
		for i in 1:mc
			Φₜₑₛₜ = basis_expansion(xtest, xtrain, σ²s[i], intercept)
			ts[:, i] = Φₜₑₛₜ * ws[:,i]
		end
		return ts
	end
	...
	function prediction(chain, x)
	    ...
	    targets = x * reduce(hcat, p.coefficients)'
	    return vec(mean(targets; dims = 2))
	end
end

# ╔═╡ b859e77b-8888-4bfc-ba50-39985ca60dee
begin
...
#   gx0   g(x0)
#
function slice_sampler(x0::Float64, g, w::Float64, m::Int64, lower::Float64, upper::Float64, gx0::Float64)
...
# ╔═╡ d5b88b35-aec0-4932-b281-42e01ca88152
begin
	function gibbsBayesFullBasisLR(xs, ts, a0, b0, c0, d0, e0=1, f0=1, mc=5000, burnin=1000; intercept= false)
		N = size(xs)[1]
		M = intercept ? N+1 : N
		# place holders or storage
		ws = zeros(M, mc)
		...
		for i in 1:(mc+burnin)
			# sample weight w
			C0 = Diagonal(αt)
			Φt = basis_expansion(xs, xs, σϕ²t, intercept)
			wt = samplew(ts, Φt, m0, C0, βt)
			# sample β
			sse = sum((ts - Φt*wt).^2)
			βt = sampleβ(sse, N, c0, d0)
			# sample α
			αt = sampleαs(wt, a0, b0)
			# sample σϕ²
			logPσ = (x) -> logPosteriorBandWidth(x, e0, f0, xs, ts, wt, βt, intercept=intercept)
			# assume the prior of bandwidth is truncated between 1e-6 to 1
			σϕ²t, _ =slice_sampler(σϕ²t, logPσ, 0.5, 1000, 1e-6, 1.0, logPσ(σϕ²t))
		...
		return ws, αs, βs, σϕ²s
	end

	function logPosteriorBandWidth(σϕ², e0, f0, X, t, w, β; intercept=false)
		Φ_ = basis_expansion(X, X, σϕ², intercept)
		logpdf(InverseGamma(e0, f0), σϕ²) + logpdf(MvNormal(Φ_ * w, sqrt(1/β)), t)
	end

end

# ╔═╡ 754ab865-b5bf-4a67-8529-c344ed44f0e8
begin
	# testing for the posterior distribution of the bandwidth
	xxs = collect(0.001:0.1:10)
	tmp = zeros(length(xxs))
	for i in 1:length(tmp)
		tmp[i] = logPosteriorBandWidth(xxs[i], 1, 1, xsamples, ts, ws[:,100], βs[100], intercept=intercept)
	end
	plot(xxs, tmp)
	print()
end

# ╔═╡ 76f16fed-7f3d-46e5-a535-399dcc63e814
begin
	wϕs, αϕs, βϕs, σϕs=gibbsBayesFullBasisLR(xsamples, ts, a0, b0, c0, d0, 1.0, 1.0; intercept= intercept)
	print()
end

# ╔═╡ f6150142-d0af-449c-ad0f-bf4661e48e5f
plot(σϕs, label="σ²ϕ")

# ╔═╡ 0573da68-3641-4e75-ac73-c224db8111ab
begin
	xs = -10:0.1:10
...
# ╔═╡ 6c57ac65-62a6-4943-96da-de92974316d0
plot(xs, rbf_basis_(xs, xsamples[1:end]', 1), label="")

# ╔═╡ ec5a971a-7ebc-4976-8af7-44111045596a
mseₘₗ = mean((Φₜₑₛₜ*wₘₗ - tₜₑₛₜ).^2)

# ╔═╡ 90fb8e21-65b8-41eb-a80e-b111a383cda9
mseₑₘ = mean((Φₜₑₛₜ*wₑₘ - tₜₑₛₜ).^2)

# ╔═╡ dfa329f7-199c-485a-a493-de82a4356a42
mse_gibbs = mean((mean(gibbsPredictions, dims=2) - tₜₑₛₜ).^2)

# ╔═╡ 9c3c67cf-a141-430f-a53b-2df21f6e56d1
mse_fullgibbs = mean((mean(gibbsFullPred, dims=2) - tₜₑₛₜ).^2)

# ╔═╡ fd536a91-4da5-4f80-a78b-d9fcad00d24b
mseᵥᵢ = mean((Φₜₑₛₜ*wᵥᵢ - tₜₑₛₜ).^2)

# ╔═╡ 4d8ebde8-3a14-42c9-9774-97b68b16675a
mse_turing = mean((turingPrediction - tₜₑₛₜ).^2)

# ╔═╡ 080675d8-4370-4ff6-955c-991b3f94afa6
begin
	step =25
	plot(xs, tₜₑₛₜ, ylim= [-0.8, 2.5], linecolor=:black, linestyle=:dash, label="Original")
	scatter!(xsamples, ts, markershape=:xcross, markersize=2.5, markerstrokewidth=0.1, markercolor=:gray, markeralpha=0.9, label="Obvs")
...
# ╔═╡ 8ee77ac4-7d81-40c5-b681-219e11f3ae5a
begin
	plot(xs, tₜₑₛₜ, ylim= [-0.8, 2.5], linecolor=:black, linestyle=:dash, label="Original")
	scatter!(xsamples, ts, markershape=:xcross, markersize=2.5, markerstrokewidth=0.1, markercolor=:gray, markeralpha=0.9, label="Obvs")
	plot!(xs, gibbsFullPred[:, 1:step:end], linecolor=:red, linestyle=:solid, lw=0.1, label="")
	plot!(xs, mean(gibbsFullPred, dims=2), linecolor=:red, linestyle=:solid, lw=2, label="FGibb")
end
